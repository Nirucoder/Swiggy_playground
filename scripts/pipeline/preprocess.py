...
def main():
    ...
    # NEW: Synthetic Discount Logic
    np.random.seed(42)
    master_df['discount'] = 0
    promo_days = master_df.sample(frac=0.3).index
    master_df.loc[promo_days, 'discount'] = np.random.choice([10, 20, 50], size=len(promo_days))
